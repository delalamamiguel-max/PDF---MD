...
export function DocumentsTable({ items }: { items: DocumentItem[] }) {
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-[var(--muted)] last:border-none">
                ...
                <td className="px-4 py-3 text-[var(--muted-foreground)]">{formatDate(item.created_at)}</td>
            ...
            {!items.length ? (
              <tr>
                <td className="px-4 py-12 text-center text-[var(--muted-foreground)]" colSpan={5}>
